...
{
  struct proc *p;
  struct pid_ns *cur = myproc()->nsproxy->pid_ns;
  struct pid_ns *par = cur->parent;
  int search_vpid;

  cprintf("name \t pid \t state \t \t extpid \t ppid \t size \t cputime \n");

  for(search_vpid = 1; search_vpid <= 500; search_vpid++){
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state == UNUSED)
        continue;
      ...
      int vextpid = 0;
      for(int i = 0; i < 4; i++){
        if(p->pids[i].pid_ns == cur)
          vpid = p->pids[i].pid;
        if(par && p->pids[i].pid_ns == par)
          vextpid = p->pids[i].pid;
      }
      ...
      if(vpid == search_vpid){
        ...
        if(p->parent){
          for(int i = 0; i < 4; i++){
            if(p->parent->pids[i].pid_ns == cur){
              vppid = p->parent->pids[i].pid;
              break;
            }
          }
        }
        ...
        cprintf("%s \t %d \t %s \t %d \t \t %d \t %d \t %d \n",
                p->name, vpid, state2str(p->state),
                vextpid, vppid, p->sz, p->cpu_time);
